...
export default function Features() {
  ...
  return <div className={` flex lg:flex-row flex-col items-center justify-center gap-6 `}>
    ...
    {insightsCards.map((e , idx)=>(
        <div key={idx} className={` text-xs p-6 w-70 h-65 rounded-xl ${e.variant=='light' ? 'bg-neutral-100 border-neutral-300 border' : 'bg-lime-400/80'} backdrop-blur-xl`}>
            <div className="w-10 h-10  rounded-xl my-2"><e.icon/></div>
            <div className="text-lg font-semibold">{e.title}</div>
            <div className="w-55">{e.description}</div>
